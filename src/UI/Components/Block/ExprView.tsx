...
import { Term } from 'FunBlocks/AST'
import { TermViewContainer } from './TermViewContainer'
import { ExprViewProps } from './Types'

const styles = require('./TermView.module')

export class ExprView extends React.PureComponent<ExprViewProps> {

  didLabelClick(e: React.MouseEvent<HTMLInputElement>) {
    e.currentTarget.select()
  }

  render() {
    const className = cn(styles.expr)
    // const className = cn('no-text-select', styles.expr, {
    //   [styles.faded]: this.props.isFaded,
    //   [styles.shaking]: this.props.isShaking,
    // })

    // Separate the props that are related solely to this view from those that should be forwarded.
    const {
      label,
      subterms,
      colorScheme,
    ...
    const subviews = subterms.map((subterm, i) => (
      <TermViewContainer
        key={ i }
        term={ subterm }
        { ...forwardedProps }
      ...
      >
        <div className={ styles.exprLabel }>
          <input
            ...
            size={ Math.max(this.props.label.length, 1) }
            readOnly={ !this.props.editable }
            onClick={ this.didLabelClick.bind(this) }
